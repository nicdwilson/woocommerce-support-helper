<?php
/**
 * Canada Post Shipping Method Exporter Class
 *
...
/**
 * Canada Post Shipping Method Exporter.
 *
 * Exports Canada Post shipping method settings for WooCommerce Blueprint.
 *
 * @package WooCommerceSupportHelper
 */
class WooCommerce_Shipping_Canada_Post implements StepExporter, HasAlias {

	/**
	 * Plugin slug for Canada Post shipping method.
	 */
	const PLUGIN_SLUG = 'woocommerce-shipping-canada-post';

	/**
	 * Method ID for Canada Post shipping method.
	 */
	...
	public function get_alias() {
		return 'wcShippingCanadaPost';
	}

	/**
	 * Export Canada Post shipping method settings.
	 *
	 * @return Step
	 */
	public function export(): Step {
		// Get all Canada Post site options.
		$site_options = $this->get_site_options();

		Logger::info( 'Canada Post Exporter: Export completed successfully' );
	...
	/**
	 * Get Canada Post site options for Blueprint export.
	 *
	 * @return array
	 */
	public function get_site_options() {
		...
		$global_settings = get_option( 'woocommerce_canada_post_settings', array() );
		if ( ! empty( $global_settings ) ) {
			$site_options['woocommerce_canada_post_settings'] = $this->sanitize_settings( $global_settings );
		}

		// Get per-method settings for each shipping zone.
		$zones_with_canada_post = $this->get_shipping_zones_with_canada_post();

		foreach ( $zones_with_canada_post as $zone ) {
			$method_settings = $this->get_method_settings_for_zone( $zone );
			if ( ! empty( $method_settings ) ) {
				$option_name                  = 'woocommerce_canada_post_' . $zone['method_instance_id'] . '_settings';
	...
	/**
	 * Get shipping zones that have Canada Post shipping method configured.
	 *
	 * @return array
	 */
	public function get_shipping_zones_with_canada_post() {
		$zones_with_canada_post = array();
	...
	public function get_method_settings() {
		...
		foreach ( $zones_with_canada_post as $zone ) {
			$method_settings = $this->get_method_settings_for_zone( $zone );
			if ( ! empty( $method_settings ) ) {
				// Categorize settings based on common Canada Post configuration patterns.
	...
	private function categorize_settings( $settings ) {
		...
		$service_keys   = array( 'service', 'domestic', 'international', 'rate', 'delivery' );
		$packing_keys   = array( 'packaging', 'box', 'weight', 'package', 'dimension' );
		$advanced_keys  = array( 'debug', 'tax', 'handling', 'free', 'insurance' );
	...
	/**
	 * Get full export data including all Canada Post configuration.
	 *
	 * @return array
	 */
	...
	/**
	 * Check if the Canada Post plugin is active.
	 *
	 * @return bool
	 */
	...
	/**
	 * Check if the current user has the required capabilities to export Canada Post settings.
	 *
	 * @return bool
	 */
