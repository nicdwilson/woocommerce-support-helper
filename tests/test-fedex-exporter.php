...
// Include the FedEx exporter class directly
require_once __DIR__ . '/../includes/shipping-methods-exporter/woocommerce-shipping-fedex/class-woocommerce-shipping-fedex.php';

echo "Testing FedEx Exporter...\n\n";

try {
    // Test 1: Instantiate the exporter
    echo "1. Testing instantiation...\n";
    $fedex_exporter = new \WooCommerceSupportHelper\WooCommerce_Shipping_Fedex();
    echo "   ✓ FedEx exporter instantiated successfully\n";
    echo "   Plugin slug: " . $fedex_exporter->get_plugin_slug() . "\n";
    echo "   Method ID: " . $fedex_exporter->get_method_id() . "\n";
    echo "   Alias: " . $fedex_exporter->get_alias() . "\n\n";

    // Test 2: Check plugin active status
    echo "2. Testing plugin active check...\n";
    $is_active = $fedex_exporter->is_plugin_active();
    echo "   Plugin active: " . ( $is_active ? 'Yes' : 'No' ) . "\n";
    if ( ! $is_active ) {
        echo "   Note: FedEx plugin is not active, but exporter can still be tested\n";
    }
    echo "\n";

    // Test 3: Test site options export
    echo "3. Testing site options export...\n";
    $site_options = $fedex_exporter->get_site_options();
    ...
    $zone_configs = $fedex_exporter->get_shipping_zone_configurations();
    ...
    $method_settings = $fedex_exporter->get_method_settings();
